...
var attributes = require('../models/attributes/e_espace');
var options = require('../models/options/e_espace');
...
router.get('/list', block_access.actionAccessMiddleware("espace", "read"), function(req, res) {
    res.render('e_espace/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("espace", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_espace", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_espace', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("espace", "read"), function(req, res) {
    ...
    models.E_espace.findOne({
        where: {
            id: parseInt(sourceId)
        },
        include: include
    }).then(function(e_espace) {
        if (!e_espace['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_espace['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_espace[subentityAlias].length; i++)
                rawData.data.push(e_espace[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("espace", "read"), function(req, res) {
    var id_e_espace = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_espace", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_espace', id_e_espace, options).then(function(e_espace) {
        if (!e_espace) {
            data.error = 404;
        ...
        /* Update local e_espace data before show */
        data.e_espace = e_espace;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_espace, "e_espace").then(function() {
            status_helper.translate(e_espace, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_espace");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_espace/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("espace", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_espace", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_espace/create_fields' : 'e_espace/create';
...
router.post('/create', block_access.actionAccessMiddleware("espace", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_espace.create(createObject).then(function(e_espace) {
        var redirect = '/espace/show?id=' + e_espace.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_espace.id).then(function() {
                        ...
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_espace.id;
        ...
        model_builder.setAssocationManyValues(e_espace, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_espace, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/espace/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/espace/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("espace", "update"), function(req, res) {
    var id_e_espace = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_espace", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_espace', id_e_espace, options).then(function(e_espace) {
        ...
        e_espace.dataValues.enum_radio = data.enum_radio;
        data.e_espace = e_espace;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_espace, "e_espace", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_espace.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_espace.dataValues = data;
                    res.render('e_espace/update_fields', e_espace.get({
                        plain: true
                    }));
                } else
                    res.render('e_espace/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("espace", "update"), function(req, res) {
    var id_e_espace = parseInt(req.body.id);
    ...
    models.E_espace.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_espace, options, req.body);
        e_espace.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_espace, req.body, updateObject, options).then(function() {

                var redirect = '/espace/show?id=' + id_e_espace;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/espace/update_form?id=' + id_e_espace);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/espace/update_form?id=' + id_e_espace);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('espace', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_espace.findOne(queryOpts).then(function(e_espace) {
        if (!e_espace)
            return res.status(404).end();

        var dustData = e_espace[option.as] || null;
...
router.get('/set_status/:id_espace/:status/:id_new_status', block_access.actionAccessMiddleware("espace", "update"), function(req, res) {
    status_helper.setStatus('e_espace', req.params.id_espace, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/espace/show?id=' + req.params.id_espace);
    });
});

router.post('/search', block_access.actionAccessMiddleware('espace', 'read'), function(req, res) {
    ...
    if(typeof req.body.shared !== 'undefined'){
        where.where = [];
        where.where.push({
            'f_partage': true
        })
    ...
    models.E_espace.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("espace", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_espace.findOne({
        ...
        e_espace['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function (aliasEntities) {
            var target = "";
            for (var i = 0; i < options.length; i++)
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("espace", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_espace.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/espace/show?id=' + idEntity + "#" + alias);
        }

        e_espace['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function () {
            var target = "";
            for (var i = 0; i < options.length; i++)
                if (options[i].as == alias)
                    {target = options[i].target; break;}
            ...
            res.redirect('/espace/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("espace", "delete"), function(req, res) {
    var id_e_espace = parseInt(req.body.id);

    models.E_espace.findOne({where: {id: id_e_espace}}).then(function (deleteObject) {
        if (!deleteObject) {
            req.session.toastr = [{level: 'error', message: 'error.404.title'}];
            return res.redirect('/espace/list');
        }
        deleteObject.destroy().then(() => {
            ...
            var redirect = '/espace/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_espace", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/espace/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/espace/list');
    });
