...
router.get('/list', block_access.actionAccessMiddleware("agenda_event", "read"), function(req, res) {
    res.render('e_agenda_event/list');
});
...
router.post('/subdatalist', block_access.actionAccessMiddleware("agenda_event", "read"), function(req, res) {
    ...
    models.E_agenda_event.findOne({
        ...
        if (!e_agenda_event['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_agenda_event['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_agenda_event[subentityAlias].length; i++)
                rawData.data.push(e_agenda_event[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("agenda_event", "read"), function(req, res) {
    var id_e_agenda_event = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_agenda_event", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_agenda_event', id_e_agenda_event, options).then(function(e_agenda_event) {
        if (!e_agenda_event) {
            data.error = 404;
        ...
        /* Update local e_agenda_event data before show */
        data.e_agenda_event = e_agenda_event;
...
router.get('/create_form', block_access.actionAccessMiddleware("agenda_event", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_agenda_event", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_agenda_event/create_fields' : 'e_agenda_event/create';
...
router.post('/create', block_access.actionAccessMiddleware("agenda_event", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_agenda_event.create(createObject).then(function(e_agenda_event) {
        var redirect = '/agenda_event/show?id=' + e_agenda_event.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        ...
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_agenda_event.id;
        ...
        model_builder.setAssocationManyValues(e_agenda_event, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                ...
                entity_helper.error(err, req, res, '/agenda_event/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/agenda_event/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("agenda_event", "update"), function(req, res) {
    var id_e_agenda_event = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_agenda_event", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_agenda_event', id_e_agenda_event, options).then(function(e_agenda_event) {
        ...
        e_agenda_event.dataValues.enum_radio = data.enum_radio;
        data.e_agenda_event = e_agenda_event;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_agenda_event, "e_agenda_event", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_agenda_event.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_agenda_event.dataValues = data;
                    res.render('e_agenda_event/update_fields', e_agenda_event.get({
                        plain: true
                    }));
                } else
                    res.render('e_agenda_event/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("agenda_event", "update"), function(req, res) {
        ...
        e_agenda_event.update(updateObject).then(function() {
            ...
            model_builder.setAssocationManyValues(e_agenda_event, req.body, updateObject, options).then(function() {
				...
				if (!view) {
					...
					redirect = '/agenda_event/show?id=' + id_e_agenda_event;
				}
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('agenda_event', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_agenda_event.findOne(queryOpts).then(function(e_agenda_event) {
        if (!e_agenda_event)
            return res.status(404).end();

        var dustData = e_agenda_event[option.as] || null;
...
router.get('/set_status/:id_agenda_event/:status/:id_new_status', block_access.actionAccessMiddleware("agenda_event", "update"), function(req, res) {
    status_helper.setStatus('e_agenda_event', req.params.id_agenda_event, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect('/agenda_event/show?id=' + req.params.id_agenda_event)
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/agenda_event/show?id=' + req.params.id_agenda_event);
    });
});

router.post('/search', block_access.actionAccessMiddleware('agenda_event', 'read'), function(req, res) {
    ...
    models.E_agenda_event.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("agenda_event", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_agenda_event.findOne({
        ...
        e_agenda_event['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function (aliasEntities) {
            var target = "";
            for (var i = 0; i < options.length; i++)
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("agenda_event", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_agenda_event.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/agenda_event/show?id=' + idEntity + "#" + alias);
        }

        e_agenda_event['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function () {
            var target = "";
            for (var i = 0; i < options.length; i++)
                if (options[i].as == alias)
                    {target = options[i].target; break;}
            ...
            res.redirect('/agenda_event/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("agenda_event", "delete"), function(req, res) {
    var id_e_agenda_event = parseInt(req.body.id);
    ...
    models.E_agenda_event.findOne({where: {id: id_e_agenda_event}}).then(function (deleteObject) {
        ...
        deleteObject.destroy().then(() => {
            ...
            var redirect = '/agenda_event/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
