            ...
            render: function (data, type, row, meta) {
                ...
                if (typeof columns[meta.col].type != 'undefined') {
                    ...
                    if (columns[meta.col].type == 'date' || columns[meta.col].type == 'datetime') {
                        ...
                        cellValue = '<span class="badge" style="background: '+statusObj.f_color+';">'+statusObj.f_button_label+'</span>';
                    }
                    else if (columns[meta.col].type == 'currency')
                        cellValue = '<span data-type="currency">' + currencyFormat(cellValue) + '</span>';
					else if (columns[meta.col].type == 'millisecond')
                        cellValue = cellValue ? Math.round(cellValue).toLocaleString() : 0;
...
function buildPieChart(widgetID, widget) {
	...
	var colors = [];
	for (var label in widget.data.labels) {
		let val = widget.data.labels[label];
		
		if (!val) widget.data.labels[label] = "Non terminé"
		
		if (val == "SUCCESS") colors.push("#00a65a");
		else if (val == "ERROR") colors.push("#dd4b39");
...
function buildBarChart(widgetID, widget) {
    var widgetCanvas = $("#"+widgetID+" .barchart")[0];
    var legend = $("#"+widgetID).data('legend');
	...
	for (var label in widget.data.labels) {
		let val = widget.data.labels[label];
		if (val == "Initialisée") colors.push("#999999");
		if (val == "A traiter") colors.push("#0073b7");
		if (val == "Succès") colors.push("#00a65a");
		if (val == "Echec") colors.push("#dd4b39");
	}
...
function buildOccupationChart(widgetID, widget) {
	var spanWaiting = $("#"+widgetID+" .waiting")[0];
    var spanOccupation = $("#"+widgetID+" .occupation")[0];

	spanWaiting.innerHTML = widget.data[0].waiting + " sec.";
}
...
function initWidgets() {
    ...
    $(".ajax-widget").each(function() {
        var widget = {
			...
			status: $(this).data('widget-status'),
        ...
        if (widget.type && widget.type == 'barchart') {
            widget.fieldType = $(this).data('field-type');
            widget.field = $(this).data('field');
        }
        if (widget.type && widget.type == 'occupation') {
            widget.fieldType = $(this).data('field-type');
            widget.field = $(this).data('field');
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var widgetID in data) {
                    ...
                    else if ((widget.type == 'stats')||(widget.type == 'custom_task'))
                        $('#'+widgetID).find('h3').text(widget.data);
                    else if (widget.type == 'piechart')
                        buildPieChart(widgetID, widget);
                    else if (widget.type == 'barchart')
                        buildBarChart(widgetID, widget);
                    else if (widget.type == 'occupation')
                        buildOccupationChart(widgetID, widget);
                }
            }
        });
}