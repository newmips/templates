...
router.get('/:id/downloadProgram', function(req, res) {
	models.E_task.findOne({
		...
		include: {
			...
			as: 'r_traitement',
			include: {
				model: models.E_programme,
				as: 'r_programme'
			}
		}
	}).then(task => {
		if (!task || !task.r_traitement || !task.r_traitement.r_programme || !task.r_traitement.r_programme.f_fichier_programme)
			return res.sendStatus(404);
		const fileField = task.r_traitement.r_programme.f_fichier_programme;
		const fileFolder = fileField.split('-')[0];
		const filePath = globalConf.localstorage+'/e_programme/'+fileFolder+'/'+fileField;
