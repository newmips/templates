...
var attributes = require('../models/attributes/e_demande');
var options = require('../models/options/e_demande');
...
// m_extranet
router.get('/extranet', block_access.isLoggedIn, block_access.moduleAccessMiddleware("extranet"), function(req, res) {
    var widgetPromises = [];
    // *** Widget module m_extranet | Do not remove ***
    Promise.all(widgetPromises).then(function(results) {
        var data = {};
        for (var i = 0; i < results.length; i++)
            for (var prop in results[i])
                data[prop] = results[i][prop];
        res.render('default/m_extranet', data);
    });
});

// m_extranet
router.post('/extranet', block_access.isLoggedIn, block_access.moduleAccessMiddleware("extranet"), function(req, res) {
    
    var createObject = model_builder.buildForRoute(attributes, options, req.body);
    //createObject = enums.values("e_demande", createObject, req.body);

    models.E_demande.create(createObject).then(function (e_demande) {
        var redirect = '/default/extranet';
        req.session.toastr = [{
        ...
        if (typeof req.body.associationFlag !== 'undefined') {
            ...
            models[capitalizeFirstLetter(req.body.associationSource)].findOne({where: {id: req.body.associationFlag}}).then(function (association) {
                ...
                if (typeof association['add' + modelName] !== 'undefined')
                    association['add' + modelName](e_demande.id);
                else {
                    var obj = {};
                    obj[req.body.associationForeignKey] = e_demande.id;
        ...
        model_builder.setAssocationManyValues(e_demande, req.body, createObject, options);

        res.redirect(redirect);
    }).catch(function (err) {
        error500(err, req, res, 'default/extranet');
    });

});


router.get('/parametrage', block_access.isLoggedIn, block_access.moduleAccessMiddleware("parametrage"), function(req, res) {
    var widgetPromises = [];
    // *** Widget module m_parametrage | Do not remove ***
    Promise.all(widgetPromises).then(function(results) {
        var data = {};
        for (var i = 0; i < results.length; i++)
            for (var prop in results[i])
                data[prop] = results[i][prop];
        res.render('default/m_parametrage', data);
    });
});
...
router.get('/home', block_access.isLoggedIn, block_access.moduleAccessMiddleware("home"), function(req, res) {
    var widgetPromises = [];
    // *** Widget call e_chauffeur stats start | Do not remove ***
    widgetPromises.push(new Promise(function(resolve, reject){
        models.E_chauffeur.count().then(function(result){
            resolve({e_chauffeur_stats: result});
        });
    }));
    // *** Widget call e_chauffeur stats end | Do not remove ***

    // *** Widget call e_voiture stats start | Do not remove ***
    widgetPromises.push(new Promise(function(resolve, reject){
        models.E_voiture.count().then(function(result){
             resolve({e_voiture_stats: result});
         });
     }));
    // *** Widget call e_voiture stats end | Do not remove ***

    // *** Widget call e_demande stats start | Do not remove ***
    widgetPromises.push(new Promise(function(resolve, reject){
         models.E_demande.count().then(function(result){
             resolve({e_demande_stats: result});
         });
     }));
    // *** Widget call e_demande stats end | Do not remove ***
