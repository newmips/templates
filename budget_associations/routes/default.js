...
router.get('/parametres', block_access.isLoggedIn, block_access.moduleAccessMiddleware("parametres"), function(req, res) {
    var widgetPromises = [];
    // *** Widget module m_parametres | Do not remove ***
    Promise.all(widgetPromises).then(function(results) {
        var data = {};
        for (var i = 0; i < results.length; i++)
            for (var prop in results[i])
                data[prop] = results[i][prop];
        res.render('default/m_parametres', data);
    });
});
...
router.get('/print/:source/:id', block_access.isLoggedIn, function(req, res) {
    ...
    models['E_'+source].findOne({
        ...
        if (source === "etat_de_caisse"){
             var where = {
                    $and: [
                           {
                             fk_id_regie_regie: dustData.fk_id_regie_regie
                           },
                           {
                              f_date: {$gte: dustData.f_debut_de_periode}
                           },
                           {
                              f_date: {$lte: dustData.f_fin_de_periode}
                           }
                    ]
                };

            models.E_recette.findAll({where: where, include: [{ all: true }]}).then(function (e_recette) {
                dustData.e_recette = e_recette;
                ...
                dustData.valeurs = valeurs;
                dustData.total_especes = total_especes;
                dustData.total_cheques = total_cheques;
                dustData.total_cesu = total_cesu;
                dustData.total_ancv = total_ancv;
                dustData.total = total;
