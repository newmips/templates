...
router.get('/list', block_access.actionAccessMiddleware("produit", "read"), function(req, res) {
    var data = {
        "menu": "e_produit",
        "sub_menu": "list_e_produit"
    };

    data.toastr = req.session.toastr;
    req.session.toastr = [];

    res.render('e_produit/list', data);
});
...
router.post('/subdatalist', block_access.actionAccessMiddleware("produit", "read"), function(req, res) {
    ...
    models.E_produit.findOne({
        where: {
            id: parseInt(sourceId)
        },
        include: include
    }).then(function(e_produit) {
        if (!e_produit['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_produit['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_produit[subentityAlias].length; i++)
                rawData.data.push(e_produit[subentityAlias][i].get({
                    plain: true
                }));
...
router.post('/display', block_access.actionAccessMiddleware("produit", "read"), function(req, res) {
    var code_barre = req.body.code_barre;
    var tab = req.query.tab;
    var data = {
        menu: "e_produit",
        sub_menu: "list_e_produit",
        tab: tab,
        enum_radio: enums_radios.translated("e_produit", req.session.lang_user, options)
    };
	...
	console.log("Code barre scanné : " + code_barre);
    models.E_produit.findOne({
		where: {
			f_code_barre: code_barre
		}}).then(function(e_produit) {
        if (!e_produit) {
            data.error = 404;
        ...
        /* Update local e_produit data before show */
        data.e_produit = e_produit;
...
router.get('/show', block_access.actionAccessMiddleware("produit", "read"), function(req, res) {
    var id_e_produit = req.query.id;
    var tab = req.query.tab;
    var data = {
        menu: "e_produit",
        sub_menu: "list_e_produit",
    ...
    entity_helper.optimizedFindOne('E_produit', id_e_produit, options).then(function(e_produit) {
        if (!e_produit) {
            data.error = 404;
            logger.debug("No data entity found.");
            return res.render('common/error', data);
...
router.get('/create_form', block_access.actionAccessMiddleware("produit", "create"), function(req, res) {
    var data = {
        menu: "e_produit",
        sub_menu: "create_e_produit",
        enum_radio: enums_radios.translated("e_produit", req.session.lang_user, options)
    };
...
router.post('/create', block_access.actionAccessMiddleware("produit", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_produit.create(createObject).then(function(e_produit) {
        var redirect = '/produit/show?id=' + e_produit.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_produit.id).then(function() {
                        ...
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_produit.id;
...
router.get('/update_form', block_access.actionAccessMiddleware("produit", "update"), function(req, res) {
    var id_e_produit = req.query.id;
    var data = {
        menu: "e_produit",
        sub_menu: "list_e_produit",
    ...
    entity_helper.optimizedFindOne('E_produit', id_e_produit, options).then(function(e_produit) {
        ...
        e_produit.dataValues.enum_radio = data.enum_radio;
        data.e_produit = e_produit;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_produit, "e_produit", true).then(function() {
            // Get association data that needed to be load directly here (loadOnStart param in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_produit.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_produit.dataValues = data;
                    res.render('e_produit/update_fields', e_produit.get({
                        plain: true
                    }));
                } else
...
router.post('/update', block_access.actionAccessMiddleware("produit", "update"), function(req, res) {
    var id_e_produit = parseInt(req.body.id);
    ...
    models.E_produit.findOne({
        ...
        e_produit.update(updateObject).then(function() {
            ...
            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_produit, req.body, updateObject, options).then(function() {

                var redirect = '/produit/show?id=' + id_e_produit;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('produit', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_produit.findOne(queryOpts).then(function(e_produit) {
        if (!e_produit)
            return res.status(404).end();

        var dustData = e_produit[option.as] || null;
...
router.get('/set_status/:id_produit/:status/:id_new_status', block_access.actionAccessMiddleware("produit", "update"), function(req, res) {
    var historyModel = 'E_history_e_produit_' + req.params.status;
    var historyAlias = 'r_history_' + req.params.status.substring(2);
    var statusAlias = 'r_' + req.params.status.substring(2);

    var errorRedirect = '/produit/show?id=' + req.params.id_produit;

    var includeTree = status_helper.generateEntityInclude(models, 'e_produit');
    ...
    models.E_produit.findOne({
        where: {
            id: req.params.id_produit
        },
        include: includeTree
    }).then(function(e_produit) {
        if (!e_produit || !e_produit[historyAlias] || !e_produit[historyAlias][0][statusAlias]) {
        ...
        models.E_status.findOne({
            where: {
                id: e_produit[historyAlias][0][statusAlias].id
            },
            include: [{
                model: models.E_status,
                as: 'r_children',
            ...
            nextStatus.executeActions(e_produit).then(function() {
                // Create history record for this status field
                // Beeing the most recent history for produit it will now be its current status
                var createObject = {}
                if (req.query.comment)
                    createObject.f_comment = req.query.comment;
                createObject["fk_id_status_" + nextStatus.f_field.substring(2)] = nextStatus.id;
                createObject["fk_id_produit_history_" + req.params.status.substring(2)] = req.params.id_produit;
                models[historyModel].create(createObject).then(function() {
                    e_produit['set' + entity_helper.capitalizeFirstLetter(statusAlias)](nextStatus.id);
                    res.redirect('/produit/show?id=' + req.params.id_produit)
                });
            }).catch(function(err) {
                console.error(err);
                req.session.toastr = [{
...
router.post('/search', block_access.actionAccessMiddleware('produit', 'read'), function(req, res) {
    ...
    models.E_produit.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("produit", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_produit.findOne({
        ...
        e_produit['get' + entity_helper.capitalizeFirstLetter(alias)]().then(function (aliasEntities) {
            ...
            e_produit['set' + entity_helper.capitalizeFirstLetter(alias)](aliasEntities).then(function () {
                var target = "";
                for (var i = 0; i < options.length; i++)
                ...
                entity_helper.synchro.writeJournal({
                    ...
                    id: idEntity,
                    target: target,
                    entityName: "e_produit",
                    func: 'set' + entity_helper.capitalizeFirstLetter(alias),
                    ids: toKeep
                });
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("produit", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_produit.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/produit/show?id=' + idEntity + "#" + alias);
        }

        e_produit['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function () {
            var target = "";
            for (var i = 0; i < options.length; i++)
                if (options[i].as == alias)
                    {target = options[i].target; break;}
...
router.post('/delete', block_access.actionAccessMiddleware("produit", "delete"), function(req, res) {
    var id_e_produit = parseInt(req.body.id);

    models.E_produit.findOne({where: {id: id_e_produit}}).then(function (deleteObject) {
        if (!deleteObject) {
            req.session.toastr = [{level: 'error', message: 'error.404.title'}];
            return res.redirect('/produit/list');
        }
